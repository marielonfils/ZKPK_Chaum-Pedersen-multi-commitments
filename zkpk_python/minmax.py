...
g_bolds=np.zeros((n,m))
h_bolds=np.zeros((n,m))
for j in range(n):
...
def generate_proof(gs,hs,h,u,g_bolds,h_bolds,Vs,vs,gammas):
    ...
    for i in range(m):
        ...
        zs_m_i= zs_m_i *z % G.q
        ys_m_i= ys_m_i *y %G.q
    zs_1_l=np.broadcast_to(zs_1,(n,m))
    ys_m_l=np.broadcast_to(ys_m,(n,m))
    zs_m_l=np.broadcast_to(zs_m,(n,m)) 
    ...
    for k in range(l):
        for j in range(m):
            c_bar = (c_bar + vs[k][j]*zs_m[j]%G.q)%G.q
    #5.3
    ...
    #step 6 - 7 -8#
    x = hashg(json.dumps({"T0":T0,"T1": T1,"T2":T2,"A": A,"S":S,"gs":gs_list, "hs" :hs_list, "h":h, "u":u, "g_bolds":g_bolds_list, "h_bolds":h_bolds_list, "Vs":Vs_list}),G.q)
    rand=0
    while x==0:
        x = hashg(json.dumps({"T0":T0,"T1": T1,"T2":T2,"A": A,"S":S,"gs":gs_list, "hs" :hs_list, "h":h, "u":u, "g_bolds":g_bolds_list, "h_bolds":h_bolds_list, "Vs":Vs_list,"rand":rand}),G.q)
        rand+=1
    #step 9#
    #9.1 - 9.2
    #TODO for all j ?
    ...
    for j in range(n):
        ...
        for i in range(m):            
            ...
            t_hats=(t_hats+ls[j][i]*rs_i[i]%G.q)%G.q
            #TODO
        rs[j,:]=rs_i
