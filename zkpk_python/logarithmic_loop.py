...
def generate_proof(gs,hs,h,u,g_bolds,h_bolds,Vs,vs,gammas):
    ...
    for j in range(l):
        ...
        for i in range(m):
            ...
            sum_t1= (sum_t1 +((a_Ls[j][i]-z)%G.q)*s_Rs_ys_m % G.q)%G.q
            sum_t1 = (sum_t1+s_Ls[j][i]*zs_m[i]%G.q+(((a_Rs[j][i]+z)%G.q*ys_m[i])%G.q)*s_Ls[j][i]%G.q)%G.q
    ...
    for j in range(l):
        ...
        sum=0
        for i in range(m):
            rs_i_i= ((rhs[i]*ys_m[i])%G.q + zs_m[i]) %G.q        
            rs_i[i]=rs_i_i
            sum = (sum + ls[j][i]*rs_i_i %G.q )%G.q
        rs[j,:]=rs_i
