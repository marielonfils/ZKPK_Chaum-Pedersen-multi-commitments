...
from utils.number import getPrime, isPrime
...
def gen_elgamal_keypair(G):
    ...
    """
    x=ElgamalSecretKey(G,randint(0,G.q-1))
    y= x.pk() #ElgamalPublicKey
    #raise NotImplemented() # TODO
    return (x,y)
...
class ElgamalSecretKey:
    ...
    def decrypt(self, c):
        ...
        """
        #raise NotImplemented() # TODO
        g_m= c.c2*pow(inverse(c.c1, self.G.p),self.x,self.G.p) %self.G.p
        return dLog(self.G.p,self.G.g, g_m )
...
class ElgamalCiphertext:
    ...
    * a*ca corresponds to plaintext multiplication (hence ciphertext
    ...
    def homomorphic_neg(self):
        ...
        #raise NotImplemented() # TODO
        return ElgamalCiphertext(
                self.G, inverse(self.c1,self.G.p),inverse(self.c2,self.G.p)
                )
    ...
    def homomorphic_mul(self, alpha):
        ...
        #raise NotImplemented() # TODO
        return ElgamalCiphertext(
                self.G,pow(self.c1, alpha, self.G.p),pow(self.c2, alpha, self.G.p))
